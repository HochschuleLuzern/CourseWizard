...
class ilObjCourseWizardGUI extends ilObjectPluginGUI
{
    ...
    */
    ...
    private function prepareActionsDropdownForOwner(\CourseWizard\CourseTemplate\Models\CourseTemplateModel $crs_template, ILIAS\UI\Factory $ui_factory) : array
    {
        global $DIC;
    ...
    protected function printProposalModal()
    {
        ...
        $repo = new \CourseWizard\CourseTemplate\CourseTemplateRepository($DIC->database());
    ...
    protected function proposeCourseTemplate()
    {
        ...
        if(isset($_POST['interruptive_items']))
        {
            ...
            $repo->updateTemplateStatus($crs_template, \CourseWizard\CourseTemplate\Models\CourseTemplateModel::STATUS_PENDING);
    ...
    protected function showMainPage(){
        ...
        foreach($crs_templates_for_overview as $category_name => $crs_list) {
            $courses = array();
            /** @var \CourseWizard\CourseTemplate\Models\CourseTemplateModel $crs_template */
            foreach($crs_list as $crs_template) {
                ...
                if($crs_template->getCreatorUserId() == $this->user->getId()
                    && $crs_template->getStatusAsCode() == \CourseWizard\CourseTemplate\Models\CourseTemplateModel::STATUS_DRAFT) {
